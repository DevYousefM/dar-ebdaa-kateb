...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title"><?php echo isset($id) ? "Update " : "Create New " ?> الكتاب</h3>
    </div>
    <div class="card-body">
        <form action="" id="product-form">
            ...
            <div class="form-group">
                <label for="sub_category_id" class="control-label">تصنيف التصنيف</label>
                <select name="sub_category_id" id="sub_category_id" class="custom-select" required>
                    <option value="" selected="" disabled="">اختر التصنبيف الأول</option>
                    <?php
            ...
            <div class="form-group">
                <label for="title" class="control-label">عنوان الكتاب</label>
                <textarea name="title" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($title) ? $title : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label for="author" class="control-label">الكاتب</label>
                <small>(<i>استخدم الفاصلة (،) للفصل بين أسماء المؤلفين</i>)</small>
            ...
            <div class="form-group">
                <label for="status" class="control-label">الحاله</label>
            ...
            <div class="form-group">
                <label for="" class="control-label">الصور</label>
    ...
    <div class="card-footer">
        <button class="btn btn-flat btn-primary" form="product-form">حفظ</button>
        <a class="btn btn-flat btn-default" href="?page=product">الغاء</a>
    </div>
</div>
<script>
    ...
    function delete_img($path) {
        ...
        $.ajax({
            ...
            error: err => {
                console.log(err)
                alert_toast("حدث خطأ اثناء حذف الكتاب", "error");
                end_loader()
            },
            success: function(resp) {
                ...
                if (typeof resp == 'object' && resp.status == 'success') {
                    ...
                    alert_toast("تم حذف الكتاب بنجاح", "success");
                } else {
                    console.log(resp)
                    alert_toast("حدث خطأ اثناء حذف الكتاب", "error");
                }
    ...
    $(document).ready(function() {
        $('.rem_img').click(function() {
            _conf("هل أنت تاكد من حذف الكتاب؟", 'delete_img', ["'" + $(this).attr('data-path') + "'"])
        })
