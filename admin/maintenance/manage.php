...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title"><?php echo isset($id) ? "Update " : "Create New " ?>انشاء حزمه جديدة</h3>
    </div>
    <div class="card-body">
        <form action="" id="package-form">
            <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
            <div class="form-group">
                <label for="title" class="control-label">اسم الحزمه</label>
                <textarea name="title" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($title) ? $title : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label for="tour_location" class="control-label">موقع الجوله</label>
            ...
            <div class="form-group">
                <label for="cost" class="control-label">السعر</label>
    ...
    <div class="card-footer">
        <button class="btn btn-flat btn-primary" form="package-form">حفظ</button>
        <a class="btn btn-flat btn-default" href="?page=responses">الغاء</a>
    </div>
</div>
<script>
    ...
    function delete_img($path) {
        ...
        $.ajax({
            ...
            error: err => {
                console.log(err)
                alert_toast("حدث خطأ اثناء حذف الصوره حاول مره اخري", "error");
                end_loader()
            },
            success: function(resp) {
                ...
                if (typeof resp == 'object' && resp.status == 'success') {
                    ...
                    alert_toast("تم حذف الصوره بنجاح", "success");
                } else {
                    console.log(resp)
                    alert_toast("حدث خطأ اثناء حذف الصوره حاول ره أخري", "error");
                }
                end_loader()
            }
        })
    ...
    $(document).ready(function() {
        $('.rem_img').click(function() {
            _conf("هل أنت متاكد من حذف هذه الصوره؟", 'delete_img', ["'" + $(this).attr('data-path') + "'"])
        })
