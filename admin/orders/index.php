					...
					<tbody>
						...
						?>
							<tr>
								...
								<td class="text-center">
									<?php if ($row['status'] == 0) : ?>
										<span class="badge badge-light">قيد الانتظار</span>
									<?php elseif ($row['status'] == 1) : ?>
										<span class="badge badge-primary">pdf</span>
									<?php elseif ($row['status'] == 2) : ?>
										<span class="badge badge-warning">خارج التوصيل</span>
									<?php elseif ($row['status'] == 3) : ?>
										<span class="badge badge-success">تم التوصيل</span>
									<?php elseif ($row['status'] == 5) : ?>
										<span class="badge badge-success">pdf تم الرفع</span>
									<?php else : ?>
								...
								<td align="center">
									...
									<div class="dropdown-menu" role="menu">
										<a class="dropdown-item" href="?page=orders/view_order&id=<?php echo $row['id'] ?>">عرض الطلب</a>
										<?php if ($row['paid'] == 0 && $row['status'] != 4) : ?>
											<a class="dropdown-item pay_order" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">وضع علامة على أنه مدفوع</a>
										<?php endif; ?>
...
<script>
	$(document).ready(function() {
		$('.delete_data').click(function() {
			_conf("هل أنت تأكد من حذف هذا الطلب؟", "delete_order", [$(this).attr('data-id')])
		})
		$('.pay_order').click(function() {
			_conf("هل أنت متأكد من جعل هذا الطلب مدفوع؟", "pay_order", [$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
	...
	function delete_order($id) {
		...
		$.ajax({
			...
			error: err => {
				console.log(err)
				alert_toast("يوجد خطأ حاول ره أخري", 'error');
				end_loader();
			},
			success: function(resp) {
				if (typeof resp == 'object' && resp.status == 'success') {
